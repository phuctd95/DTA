...
// 0 for requirement function, 1 for top-k and 2 for dual
int u_bound_fun;
vector<bool> v_zeros;
long long full_sketch;
double sigma_l = 0, sigma_r = 1;
vector<int> n_degree;
double dtmp_1, dtmp_2;
...
void EXHAUST(std::function< std::vector<int32_t>(int32_t &,int &, std::vector<bool> &, std::vector<bool> &) > sampler, std::vector<int> & seeds, int t, float epsilon, float delta, double normalized)
{
        double deg = 1;
        long long int numSuccessSamples = 1;
        long long int numTotalSamples = 1;
        ...
        vector<bool> cseedmark(numNodes,false);
        double c = 0.1;
        double u = log(numNodes*(log(Tmax)/log(1+c)));
        double ubound = numNodes;
        double lognk = 0;
        double epsilon2 = sqrt((1-1/exp(1))*(lognk + log(7/delta)))/((1-1/exp(1))*sqrt(log(7/delta))+sqrt((1-1/exp(1))*(lognk + log(7/delta))))*epsilon;
        lambda = (2+2*epsilon2/3)*(lognk + log(7/delta))/(epsilon2*epsilon2);
	...
	cout << "Compute bound" << endl;
        ...
        double N = ceil(pow(1+c,ceil(log(numTotalSamples)/log(1+c))));
        ubound = upperBound(u,numTotalSamples,N,zmax/numTotalSamples);
        printf("Upperbound: %0.2lf\n",ubound*normalized);
}
...
int main(int argc, char ** argv)
{
	...
	tmp = op.getPara("-u");
	if (tmp == NULL){
		cout << "STOP: Please provide upper-bound function by adding one of these options: '-u 0' for requirement function, '-u 1' for top-k and '-u 2' for dual" << endl;
		exit(1);
	} else {
		u_bound_fun = atoi(tmp);
		if (u_bound_fun > 2 || u_bound_fun < 0){
			cout << "STOP: Please provide upper-bound function by adding one of these options: '-u 0' for requirement function, '-u 1' for top-k and '-u 2' for dual" << endl;
			exit(1);
		}
	}
	...
	if (strcmp(model, "LT") == 0){
		...
		if (strcmp(alg, "SRA") == 0){
			SRA(std::bind(&IM_LTSampler::polling, &sampler, _1, _2, _3, _4), seeds, t, epsilon,delta,n*1.0);
		} else if (strcmp(alg, "DTA") == 0){
			DTA(std::bind(&IM_LTSampler::polling, &sampler, _1, _2, _3, _4),seeds,t,epsilon,delta,n*1.0);
		} else if (strcmp(alg, "UBOUND") == 0){
                        EXHAUST(std::bind(&IM_LTSampler::polling, &sampler, _1, _2, _3, _4), seeds, t, epsilon,delta,n*1.0);
                }
		...
		if (strcmp(alg, "SRA") == 0){
                        SRA(std::bind(&IM_ICSampler::polling, &sampler, _1, _2, _3, _4), seeds, t, epsilon,delta,n*1.0);
                } else if (strcmp(alg, "DTA") == 0){
                        DTA(std::bind(&IM_ICSampler::polling, &sampler, _1, _2, _3, _4),seeds,t,epsilon,delta,n*1.0);
                } else if (strcmp(alg, "UBOUND") == 0){
                        EXHAUST(std::bind(&IM_ICSampler::polling, &sampler, _1, _2, _3, _4), seeds, t, epsilon,delta,n*1.0);
                }
	} else {
